...
function productPage(data) {
    data.forEach(product => {
        // L'affichage dépendra de l'id figurant dans l'URL
        if (window.location.href.indexOf(product._id) > -1) {
            const { name, _id, price, description, imageUrl } = product
            let productChoice = product.lenses || product.colors || product.varnish
            // création de la page innerHTML
            let productDislay = new Product();
            productDislay.render(product)
